    ...
    for cand in candidates:
        for t, lo, la, al in samples:
            if not cand.alive_at(t, grace=0.5):
                continue
            pos = cand.position_at(t)
            if pos is None:
                continue
            d = geo.slant_range(lo, la, al, pos[0], pos[1], pos[2])
            if d < best_d:
...
def analyze_weapons(rec: Recording, destructions: Optional[Dict[str, Destruction]] = None) -> WeaponReport:
    ...
    weapons = sorted(
        (tr for tr in rec.tracks.values() if tr.category == "weapon"),
...
def _gun_bursts(
    ...
    for launcher_id, rounds in gun_rounds.items():
        ...
        for grp in groups:
            ...
            probe = []
            for r in (grp[0], grp[len(grp) // 2], grp[-1]):
                probe.extend(r[1])
